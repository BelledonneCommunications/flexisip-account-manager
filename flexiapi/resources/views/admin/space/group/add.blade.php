...
@section('content')
    <header>
        ...
        <form method="POST" action="{{ route('admin.spaces.groups.accounts.attach', [$space, $group]) }}"
            ...
            <select name="account_ids[]" class="list_toggle" data-list-id="gp.a{{ $group->id }}"></select>
    ...
    <div>
        <form class="inline" method="POST" action="{{ route('admin.spaces.groups.accounts.attach', [$space, $group]) }}"
            ...
            <div class="oppose">
                <a class="btn "
                    onclick="Utils.clearStorageList('gp.a{{ $group->id }}'); document.querySelector('form[name=groups_accounts_attach]').submit()">
                    <i class="ph ph-plus"></i>
                    {{ __('Add') }} <span class="list_toggle" data-list-id="gp.a{{ $group->id }}"></span> {{ __('Members') }}
                </a>
            </div>
        </form>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach ($accounts as $account)
                <tr>
                    <td>
                        <input class="list_toggle" type="checkbox" data-list-id="gp.a{{ $group->id }}"
                            data-id="{{ $account->id }}">
                    </td>
                    <td>
                        {{ $account->identifier }}

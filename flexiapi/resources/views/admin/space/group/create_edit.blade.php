...
@section('content')
    ...
    <header>
        @if ($group->id)
            ...
            <input form="create_edit_group" class="btn" type="submit" value="{{ __('Update') }}">
        @else
            <h1><i class="ph ph-phone-incoming"></i> {{ __('New group call') }}</h1>
            <input form="create_edit_group" class="btn oppose" type="submit" value="{{ __('Create') }}">
        @endif
    </header>
    ...
    <form method="POST" id="create_edit_group" 
    ...
    >
        ...
        <div class="select">
            <select name="strategy">
                @foreach (App\Group::STRATEGIES as $key => $label)
    ...
    @if ($group->id)
        ...
        <form method="POST" action="{{ route('admin.spaces.groups.accounts.detach', [$space, $group]) }}" name="groups_accounts_destroy" accept-charset="UTF-8">
            ...
            <select name="account_ids[]" class="list_toggle" data-list-id="gp.d{{ $group->id }}"></select>
            <input type="hidden" name="group_id" value="{{ $group->id }}">
        </form>

        <form method="GET" action="{{ route('admin.spaces.groups.edit', [$space, $group]) }}" name="groups_accounts_search" accept-charset="UTF-8">
            @csrf
            <div>
                <h2 class="center-flex"><i class="ph ph-users-three"></i>{{ __('Members') }} @if (count($accounts) == 0)
                    <span class="badge badge-error">{{__('No members')}}</span>
                @else
        ...
        <table class="large">
                ...
                <tr>
                    ...
                    <th>{{ __('SIP Adress') }}
                        <a class="btn small tertiary oppose"
                            onclick="Utils.clearStorageList('gp.d{{ $group->id }}'); document.querySelector('form[name=groups_accounts_destroy]').submit()">
                            <i class="ph ph-trash"></i>
                            {{ __('Remove') }} <span class="list_toggle" data-list-id="gp.d{{ $group->id }}"></span>
                        </a>
                    </th>
                </tr>
            </thead>
            <tbody>
                ...
                @foreach ($accounts as $account)
                    <tr>
                        <td><input class="list_toggle" type="checkbox" data-list-id="gp.d{{ $group->id }}" data-id="{{ $account->id }}"></td>

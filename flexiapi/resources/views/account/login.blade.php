...
@section('content')
    <section>
        <h1 style="margin-bottom: 3rem;"><i class="ph ph-hand-waving"></i>
            {{ __('Welcome on :app_name', ['app_name' => space()->name]) }}</h1>
        ...
        @if (space()->isSso())
            ...
            @include('parts.errors', ['name' => 'ssoNotExist'])
        @else
            <form style="margin-top: 3rem; margin-bottom: 3rem;" method="POST" action="{{ route('account.authenticate') }}"
                ...
                <div>
                    <input placeholder="{{ __('Your password') }}" required="" name="password" type="password"
                        value="">

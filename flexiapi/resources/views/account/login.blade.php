            ...
            <form style="margin-top: 3rem; margin-bottom: 3rem;" method="POST" action="{{ route('account.authenticate') }}" accept-charset="UTF-8">
                ...
                <div>
                    @if (space()->phone_registration)
                        <input placeholder="{{ __('Username or Phone') }}" required="" name="username" type="text" value="{{ old('username') }}">
                        <label for="username">{{ __('Username') }}</label>
                    @else
                        <input placeholder="username" required="" name="username" type="text" value="{{ old('username') }}">
                        <label for="username">{{ __('Username') }}</label>
                    @endif
